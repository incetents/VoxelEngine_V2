		...
		if (locations.include != std::string::npos)
		{
			...
			for (auto& include : includes)
			{
				...
				if(output_vertex.active)
					output_vertex.include += GlobalFiles.GetFile(include);
				
				if(output_geometry.active)
					output_geometry.include += GlobalFiles.GetFile(include);
				
				if(output_fragment.active)
					output_fragment.include += GlobalFiles.GetFile(include);
			}
		}
